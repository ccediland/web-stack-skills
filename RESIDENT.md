...
status: Sprint v1 en curso. Phases A y B0 mergeadas a main (`d1591e0`, `ff6552f`). Phase B1 EJECUTADA — `brand-canon-ingest` (#9) autorada, validada, empaquetada y registrada (9 skills en plugin.json); serializador C-1 y transform cubicBezier verificados contra `furever-brand` real; ratificación Stage-10 del canon v2 sigue PENDIENTE (bloquea C, no B2). SIGUIENTE = B2: validación mecánica de las 9 (+ smoke admin Sveltia + descongelar descriptions "Astro 6").
---
...
- Phase B1 (`brand-canon-ingest` #9): EJECUTADA en rama `claude/b1-brand-canon-ingest` — bundle + registro + frontera bilateral en #1; serializador y transform verificados contra `furever-brand` real (su board `run-gates.mjs` corrió ALL-GREEN como precondición).
- Pendiente de B2: smoke-test del admin de Sveltia (salto de 24 minors) + descongelar/actualizar las descriptions que aún dicen "Astro 6" (6 de 8 viejas) junto con el triggering test.
- Pendiente externo (Carlos): ratificación Stage-10 del canon v2 de `furever-brand` — NO registrada en B1 (el mecanismo condicional del prompt no disparó); bloquea el arranque de C, no B2.
- Siguiente: B2 — validación mecánica de las 9 (install ambos scopes + triggering test, ejes CSP/backgrounds/brand) → tag v1.0.0-rc.
