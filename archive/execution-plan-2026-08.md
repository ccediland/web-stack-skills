...
status: "ARCHIVED — CLOSED AT v2.0.0 (2026-08-18). The full-catalog roadmap this plan drove is DONE: 19 skills shipped across five capability waves plus the client layer (client-discovery, archetype recipes, marketplace maturity — versioning policy, drift calendar §12, installer restructure to plugin/, upstream suggestions formalized); the Furever fixture closed as a permanent test bench behind four CI gates. Final triggering 32/32 first-pass over a 21-description surface. This document is now history — nothing here is maintained."
phase: "CLOSED — v2.0.0 shipped, roadmap complete. The living source of truth is RESIDENT.md; maintenance runs on its §12 drift calendar."
home_base: "chat (claude.ai) for adjudication only; all execution and analysis in Claude Code (Fable 5)"
next_action: "None — the roadmap is closed. Future work = drift runs (RESIDENT §12) and new-skill burns against the permanent fixture, under the versioning policy in CLAUDE.md."
...
- W4 — RESUELTO Y LOCKEADO (chat, 2026-08-18; ejecutado en Run B tal cual): triage completo ejecutado — 3 skills delgadas (#16–#18) + 4 absorciones + 2 recipes + 1 frontera. El único fork abierto (c3: gate de sustancia de conversion-patterns) se resolvió EN el run: SOBREVIVIÓ como skill (razonamiento en RESIDENT §9/§11).
- Forks de W5 (Run A entregada 2026-08-18, PENDIENTES de lock): client-discovery (forma, formato de intake, hogar de la matriz de factibilidad, registro de deferrals, nombre final) · set de arquetipos de composition recipes · marketplace maturity (política de versionado, calendario de drift §12, fix del installer bloat = mover el plugin a subdirectorio [verificado: no existe mecanismo de ignore], upstream suggestions formalizadas) · cierre del fixture como banco permanente. Decision sheet en el reporte al chat.
...
## Next actions
1. Chat: lockear los forks de W5 desde la decision sheet del Run A (client-discovery + arquetipos + maturity + cierre del fixture).
2. Code (W5 Run B): construir la capa cliente según locks; política de versionado + calendario de drift (§12 del RESIDENT); aplicar el fix del installer bloat; upstream suggestions formalizadas; tag v2.0.0; archivar este plan.
...
## Session log
### 2026-08-18 — W5 Run B ejecutada (SHIP v2.0.0) — CIERRE DEL ROADMAP — Claude Code
Locks de la decision sheet aplicados tal cual, cero reversiones. `client-discovery` #19 (SKILL.md 4 fases + question-bank/brief-schema/intake-form + worked example del burn RETROACTIVO sobre el material real de Furever, sanitizado para catálogo público; description 1018 con fronteras a Claude Design y scoper). 3 recipes de arquetipo al playbook bajo contrato PROVEN/DERIVED evolucionado honestamente (lead-gen landing / catalog+self-edit / editorial-story, cada una con mapa de evidencia por ingrediente y lista de no-confirmado; consultoría-AI PODADA; client-portal = FRONTERA documentada). Maturity: política de versionado (sección CLAUDE.md), §12 calendario de drift (17 watches), restructure a `plugin/` (payload sin docs internos, LICENSE copiada por compliance MIT; install re-test 19/19), upstream-suggestions.md (S-1..S-4, S-4 degradada a robustez por evidencia del jueceo). Fixture → banco permanente (sección en su RESIDENT). Triggering final 32/32 a la primera (8 jueces ciegos, superficie 21): obligados discovery 3/3 CLEAR, mockup→NONE CLEAR, brand-discovery→scoper CLEAR, 5 NARROW todos con runner-up correcto (fronteras por diseño). Ship v2.0.0 + este plan ARCHIVADO.

### 2026-08-18 — W4 Run B ejecutada (SHIP v1.4.0) + W5 Run A — Claude Code
Lock del triage tal cual; c3 resuelto en-run: conversion-patterns SOBREVIVIÓ su gate de sustancia (autorada bajo amenaza de degradación — el resultado: cero claims cuantitativos, secuencia instrumentación-first, slots canon-gated). 3 skills delgadas + 4 absorciones (las exclusiones "view transitions"/"component styling" de motion/tokens INVERTIDAS en sus descriptions y re-juecedas) + 2 recipes; validate 18/18; backlog de capacidad COMPLETO. Fixture: 4ª puerta visual (determinismo por `reducedMotion: 'reduce'` — los fallbacks a11y son el estado fotografiable; baselines nacidas en CI por dispatch+artifact), theme morph con startViewTransition (runa 1 raw, cero deps), pase de conversión (3 hallazgos reales aplicados: nav→contacto, ask de cierre, tel:), `/aviso-de-privacidad/` del template legal (LHCI 7 URLs, axe 9 páginas), auth = stub honesto (Access exige owner). Triggering 31/32 + gap Rive-vs-Lottie (patrón léxico reincidente: la description anunciaba la EXCLUSIÓN, no la decisión) → fix → re-test 3/3. Ship v1.4.0. W5 Run A: sheet A–D con el fix del bloat verificado contra docs primarias (no hay ignore; source a subdirectorio es el patrón).
Siguiente: chat lockea forks de W5 → Run B (v2.0.0, cierre del roadmap).
