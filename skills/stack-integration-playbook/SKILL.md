...
description: Composition authority for the web-stack catalog — how the skills compose into ONE site, how the site plugs into the stack, and its capability recipes. Use when composing multiple skills, picking a brief's subset, sequencing a full build, capturing leads, measuring the site, speeding up navigation, or drafting the privacy notice. Trigger on build the full site, in what order do I apply these skills, wire the stack together, add a contact form, add analytics, track conversions, measure leads or real-user vitals, prefetch links, speculation rules, aviso de privacidad, or privacy notice template. Provides the canonical order, the seams, the stack map, and the recipes — forms (Astro Action, Turnstile), cookieless analytics (Umami), prefetching, and the legal template (LFPDPPP). Not for single capabilities — CSP goes to web-security-headers, CI gates to perf-ci-gates, A/B and redirects to edge-logic, external data to data-layer; every other capability has its owning skill and this playbook only composes them.
---
